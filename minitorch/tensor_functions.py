...
class Mul(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        return a.expand(grad_a), b.expand(grad_b)
...
class Sigmoid(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tensor:
        (b,) = ctx.saved_values
        one = b._ensure_tensor(1.0)
        one_minus_b = b.f.add_zip(one, b.f.neg_map(b))
        sig_grad = b.f.mul_zip(b, one_minus_b)
        return b.f.mul_zip(grad_output, sig_grad)
...
class Exp(Function):
    @staticmethod
    def forward(ctx: Context, t1: Tensor) -> Tensor:
        exp_result = t1.f.exp_map(t1)
        ctx.save_for_backward(exp_result)
        return exp_result
...
class Permute(Function):
    ...
    def backward(ctx: Context, grad_output: Tensor) -> Tuple[Tensor, float]:
        ...
        inv_order = np.zeros(len(order_list))
        for i, o in enumerate(order_list):
            inv_order[o] = i
        return grad_output.permute(*inv_order), 0.0 # 0 for the gradient of order
