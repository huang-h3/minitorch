...
def test_sigmoid(a: float) -> None:
    ...
    * It is  strictly increasing.
    """
    if not (0.0 <= sigmoid(a) <= 1.0):
        raise ValueError("sigmoid is not between 0.0 and 1.0")
...
def test_other() -> None:
    ...
    """
    assert_close(add(2.0, 3.0), add(3.0, 2.0))
    assert_close(neg(neg(2.0)), 2.0)
    assert_close(mul(2.0, 3.0), mul(3.0, 2.0))
    assert_close(max(2.0, 3.0), max(3.0, 2.0))
    assert_close(eq(2.0, 3.0), eq(3.0, 2.0))
...
def test_sum_distribute(ls1: List[float], ls2: List[float]) -> None:
    ...
    """
    sum_1 = ls1 + ls2
    sum_2 = addLists(ls1, ls2)
    assert_close(sum(sum_1), sum(sum_2))
