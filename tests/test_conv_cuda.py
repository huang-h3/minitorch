...
def test_conv(input: Tensor, weight: Tensor) -> None:
    minitorch.grad_check(minitorch.Conv2dFunCuda.apply, input, weight)
...
def test_conv2() -> None:
    ...
    out = minitorch.Conv2dFunCuda.apply(t, t2)
    out.sum().backward()

    minitorch.grad_check(minitorch.Conv2dFunCuda.apply, t, t2)
